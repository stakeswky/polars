...
pub(super) fn coerce_comparison_literal(
    ...
    fn matching_and_supported_dtype_class(dtype_lhs: &DataType, dtype_rhs: &DataType) -> bool {
        (dtype_lhs.is_integer() && dtype_rhs.is_integer())
            || (dtype_lhs.is_datetime() && dtype_rhs.is_datetime()
    ...
    if get_numeric_upcast_supertype_lossless(dtype_lhs, lit_rhs.dtype()).as_ref() == Some(dtype_lhs)
    {
        return Some(ReplaceLit(
            lit_rhs
                .clone()
                .cast_with_options(dtype_lhs, CastOptions::Overflowing)
                .unwrap(),
        ));
    }
    ...
    if let Some(lit_rhs_casted) = lit_rhs_casted {
        ...
        // have an effect on LtEq and Eq comparisons. Here, we compute when instead casting to the
        // type with higher precision, the value corresponding to the next multiple of the type that
        // was casted from, then subtract 1 from this value.
        //
